...
let handler = async (m, {
    ...
    naze.werewolf = naze.werewolf ? naze.werewolf : {};
    const ww = naze.werewolf ? naze.werewolf : {};
    const data = ww[chat];
    const value = args[0];
    const target = args[1];
    ...
    if (value === "create") {
        ...
        if (ww[chat].cooldown > 0) {
            if (ww[chat].time === "voting") {
                ...
                addTimer(chat, ww);
                return await run_vote(naze, chat, ww);
            } else if (ww[chat].time === "malem") {
                clearAllVote(chat, ww);
                addTimer(chat, ww);
                return await run_malam(naze, chat, ww);
            } else if (ww[chat].time === "pagi") {
                clearAllVote(chat, ww);
                addTimer(chat, ww);
                return await run_pagi(naze, chat, ww);
            }
        }
        if (ww[chat].status === true)
        ...
        for (let i = 0; i < ww[chat].player.length; i++) {
            ...
            if (ww[chat].player[i].role === "werewolf") {
                if (ww[chat].player[i].isdead != true) {
                    var text = `Hai ${naze.getName(
              ww[chat].player[i].id
            )}, Kamu telah dipilih untuk memerankan *Werewolf* ${emoji_role(
              "werewolf"
            )} pada permainan kali ini, silahkan pilih salah satu player yang ingin kamu makan pada malam hari ini\n*LIST PLAYER*:\n${list2}\n\nKetik *.wwpc kill nomor* untuk membunuh player`;                 
                    await naze.sendMessage(ww[chat].player[i].id, {
                        text: text,
                        mentions: player,
                    });
                }
